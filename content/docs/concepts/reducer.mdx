...
A buffer is the one thing freezing can't cover, because Luau has no way to freeze one. State can hold
buffers and Ledger copies them rather than sharing them, so writing into one won't reach another
server or another session. It will still change the state you're holding, and the fold won't agree
with itself afterwards. Build a new buffer instead of writing into the one you were given.
