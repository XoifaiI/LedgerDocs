...
`Default` can't hold a buffer, and `Ledger.New` refuses one. Every key that has not stored a buffer
yet reads the same `Default`, and that one cannot be copied out of the way, so a single key writing
into it would change what every other key reads. Leave the field out of `Default` and build the buffer
in your reducer on the first op that needs it.
