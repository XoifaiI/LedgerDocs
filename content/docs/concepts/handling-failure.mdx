...
Generate a new id on the retry and you have no protection at all. That is the whole reason
`Once` exists.

## Unresolved

This is the one that costs money if you get it wrong. Ledger doesn't know whether the write applied.
It does not mean no. Treat it as a refusal and you will eventually refuse something that went
through.
...
`Spent` reads like success and is not. Nothing was applied.
...
The answer that means "already happened" is `true`. Call `Transfer` again with an id that delivered,
