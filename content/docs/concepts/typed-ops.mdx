...
To move that to the build as well, annotate the state with `Ledger.Frozen`. The op stays unannotated
and still narrows:
...
`table.clone` keeps the read only types, so a clone is still read only one level down. That is how
it behaves at runtime too. Give each level you change its own clone, and cast it to the shape you
write:

```luau
local Next = table.clone(State) :: Hero
local Stats = table.clone(State.Stats) :: { Level: number }
Stats.Level += 1
Next.Stats = Stats
return Next
```

Arrays and maps stay writable in the type, and so does what they hold, because Luau can't make an
indexer read only yet. A write into one still throws while you play.
