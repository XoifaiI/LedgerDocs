...
A reducer that throws is a bug. It is not a refusal, and where the error surfaces depends on which
method you called. See [check the fields](/docs/concepts/reducer#check-the-fields).
...
function Handlers.Hatch(State: Profile, Op: Ledger.Op): Profile?
	...
	Next = SetPath(Next, { "Pets", Op.PetId }, { Species = Op.Species, Level = 1, Xp = 0, Locked = false })
...
function Handlers.Fuse(State: Profile, Op: Ledger.Op): Profile?
	...
	local First = State.Pets[Op.PetIds[1]]
	if First == nil then
		return nil
	end
	...
	for _, Id in Op.PetIds do
		...
		if Pet == nil or Counted[Id] then
			return nil
		end
		if Pet.Species ~= First.Species or Pet.Level ~= First.Level then
			return nil
		end
		if Pet.Locked or State.Equipped[Id] then
			return nil
		end
		Counted[Id] = true
	end
	...
	Next = SetPath(Next, { "Pets", Op.PetId }, {
...
Six things in there are worth pointing at.
...
**`Counted` in `Fuse` is the one that matters.** Without it, `PetIds = { "a", "a", "a" }` passes every
