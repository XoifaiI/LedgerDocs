...
The gamepass example above is a case in point. `pass:{Pass}` reads like it is doing the work, but
what actually stops a second unlock a year later is the reducer refusing when the pass is already
owned. The `Once` name only stops the retries.
