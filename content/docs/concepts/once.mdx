...
Ledger files each name under the day it was applied. The next time any name is written to that key,
each day older than 30 days goes, so a name lasts 30 to 31 days. So the set is a rolling window
