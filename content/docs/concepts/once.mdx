...
A gamepass is not a developer product. There is no `ProcessReceipt` and no receipt id. Roblox owns
the fact that the player has the pass, so never store that. Store what you gave them for it.

Ask `UserOwnsGamePassAsync` for ownership. Two things about it will cost you money if you miss them.

<Callout type="warn">
  `UserOwnsGamePassAsync` throws when the request fails, and it fails often under load. Wrap it in a
  `pcall` and never turn a thrown call into `false`. `false` means "does not own it", and handing
  that to a player who paid takes their pass away.
</Callout>

Second, Roblox caches the answer per player, per pass, per server, and the cache does not update when
the player buys one during the session. So take `PromptGamePassPurchaseFinished` as the answer for
the rest of that session.

Both point the same way. Keep what you know per session, and treat a failed check as "don't know":
...
`nil` is the answer that matters. Ask again in a moment. Don't remove a perk, don't hide the button,
and don't prompt them to buy something they already own.

The event only fires on the server that prompted. A player who buys on one server and hops to another
gets a fresh read there, which by then is correct.

Then the grant, with a `Once` name so a retry can't pay twice:
...
Keep the real guard in the reducer. A `Once` name is forgotten after 30 days and a gamepass is
forever, so the name only stops the retries. What stops a second grant a year later is your own
state:
...
`Op.Pass` is a string. A table keyed by numeric ids is an array with gaps and a datastore cannot
store it. See [What you can store](/docs/concepts/the-fold#what-you-can-store).

The event covers a purchase made here. It does not cover one made on another server, or on the
website, or before you shipped the pass. So check on join as well:
...
`== true`, so a failed check does nothing this time. `GrantPass` is safe to call for a pass they
already have, because the reducer refuses it, so there is no need to work out which ones are new.
