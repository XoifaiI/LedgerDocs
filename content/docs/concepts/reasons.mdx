...
`Release` answers it too, while a [`Grant`](/docs/guides/reservations) is handing those units to
another key. They are not there to give back, and the grant may already have delivered them. Recovery
finishes it one way or the other.
