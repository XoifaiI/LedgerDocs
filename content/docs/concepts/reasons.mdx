...
For a reservation it means the units that name held have gone. They were confirmed, released or given
back, and the name cannot set anything aside again. `Grant` also answers `Spent` once the hold has run
out, and leaves the units where they are for recovery.
