...
A hold lasts 15 minutes by default, not the 30 days an applied name lasts. `Hold` sets the time per
reservation, up to just under 30 days. A hold lives in MemoryStore, so it costs the key nothing and
runs out on its own. One key holds 256 at once.
