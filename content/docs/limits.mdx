...
near the 2 MB cap then spends its whole minute on autosaves, and a `Commit` waits behind them. Keep
an active player key under about 1 MB.

A key many servers write to meets the same limit sooner. Once it has taken 2,048 ops, its list of
seen op ids alone is about 55 KB. That key takes about 70 writes a minute before its state counts.
...
without bound. Ledger keeps the names in one bucket per day. Every time a name is written, each day
older than 30 days is dropped, whichever kind of name it holds. A name lasts 30 to 31 days.

A write copies only the bucket for its own day. The cost of a write does not grow with the names on
the key.

One name is about 37 bytes when Ledger picked the id, and more if you named the transfer yourself. A
rolling 30 day window is nowhere near the state cap for any real player. A name also carries a short
fingerprint of what it meant. The fingerprint lets a name reused for a different amount answer
[`Spent`](/docs/concepts/reasons) instead of `true`.

`ClearDelivered` runs that sweep early. It cannot drop a delivery id sooner than 30 days. A sender
can ask for a refund up to that point, and the refund needs the evidence that the delivery happened.

A key many players write to fills at the rate all of them write to it. Every `Tx` leg and every
`Transfer` adds one name. One name is about 37 bytes and lasts 30 days. That is about 1,900 names a
day on one key before the names alone fill the state cap. A transfer into or out of a full key then
answers [`Full`](/docs/concepts/reasons), and the money stays where it was. The key still takes
other writes, and it takes transfers again as old names run out.
...
that bump the tally, divided by 80. The default of 16 covers about 1,300 servers. Without
