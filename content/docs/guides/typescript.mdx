...
The state your reducer is handed is frozen at runtime. `Ledger.Frozen` says so in the type, at every
depth, arrays and maps included. Annotate the state with it and a write that would throw while you
play stops the build instead:
