...
fleet, to about one shard per 80 servers that bump the total. See
[Limits](/docs/limits#reservations-and-totals). Never lower it on a live store. The bumps on the top
shards then leave every total.
