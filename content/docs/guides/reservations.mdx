...
so a retry is safe. Once those units have gone, whether they were confirmed, released or given back,
the same name answers [`Spent`](/docs/concepts/reasons). A reservation name is a handle on live units,
not a receipt.
...
A reservation nobody confirms would hold its unit forever. A reservation holds for 15 minutes, and the
next `Reserve` on that key gives back anything older than 25. The extra 10 minutes is the margin that
stops a hand over racing the refund.
...
One key holds 256 reservations at once. Past that `Reserve` answers
[`Refused`](/docs/concepts/reasons), which in practice means they are being made faster than they are
being confirmed or released. Erasing a key hands over any reservation with a `To` before the key
goes, so a deletion doesn't take units somebody else was owed.
...
The hold is a deadline for `Grant` and not for `Confirm`. Past it `Grant` answers
[`Spent`](/docs/concepts/reasons) and leaves the units set aside for recovery. A late hand over could
put them on both keys. `Confirm` still spends them, as long as nothing has given them back yet.
...
Don't use the reservation name to work out whether a purchase already happened. Once the key compacts,
a repeat `Confirm` answers [`Unresolved`](/docs/concepts/reasons). The state can't tell you either. A
confirmed, a released and an expired reservation all leave the same absence. Put a
[`Once`](/docs/concepts/once) name on the op that grants the item and ask `DidApply`.
