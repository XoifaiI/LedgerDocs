...
`Erase` deletes the key outright. This is your GDPR button.
...
Before deleting, it checks for money set aside by an in flight transfer and hands it over first, so a
normal erase doesn't destroy anything that was on its way to someone. If it can't finish the
handover it warns with how much was lost, and you'll have to pay the receivers back yourself.
...
The delete is still a version, so `History` will show it with `Deleted = true` and the data is
