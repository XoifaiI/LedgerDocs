...
Money on its way *in* is the harder direction, because another server can be delivering to this key
at the moment you erase it. So an erase leaves a tombstone in place of the record rather than
removing the key. For the next 8 days anything sent to that key is turned away with `Refused`, the
sender keeps the money set aside, and the ordinary refund gives it back to them. After that the
tombstone goes cold and the key behaves like any other, by which time nothing can still be in flight.

The tombstone is a timestamp and nothing else, no player data, and any ordinary write clears it. If
you want the key gone for real, call `Erase` again once the tombstone is cold and it removes it.
...
The erase is still a version, so `History` will show it and the data is
recoverable through `PeekVersion` for 30 days. Roblox keeps that history whatever Ledger does, so
know about it before you erase someone for a legal reason.
