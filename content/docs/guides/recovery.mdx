...
First it passes on the money that this key was part way through sending. If a receiver does not take
it, `Erase` answers `Busy` and changes nothing. Call it again later. A transfer that cannot be
delivered is given back once it is 8 days old, so an `Erase` after that goes through.
...
no other player data. To remove the key outright, call `Erase` again after the 8 days. If the key
still holds names from the last 30 days, that erase keeps the tombstone and warns. A refund may still
have to check those names. Call `Erase` again after they run out.
