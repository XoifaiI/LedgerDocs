...
A GUID is fine in the second line. It is the same GUID every time, because it was made once. What
breaks is making one at the call site. Each attempt is then a different transfer and Ledger has
nothing to match it against, so the money moves again.

`TradeId` is whatever already names that trade. If nothing names it yet, make the id when the trade
opens and hold it there, next to the two players and the offer. An order id, a match id and a receipt
id all work the same way. Never build one from the clock, and never build one from the amount or the
two keys, because the same two players trading the same amount twice is a real thing.

[Where the id comes from](/docs/guides/transactions#where-the-id-comes-from) works through the three
cases and what has to outlive what.
...
<Callout type="warn">
  ...
  sent to the key and answers `Refused`, so the sender gets it back instead of losing it. That lasts
  until the next write on the key, or 8 days, whichever comes first. If the handover itself fails, it
  warns with how much was lost and you'll have to pay the receivers back yourself. See
