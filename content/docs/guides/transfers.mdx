...
<Callout type="warn">
  `Store:Erase(Key)` hands over any money still in flight before it deletes, so a normal erase won't
  destroy it. If the handover itself fails, it warns with how much was lost and you'll have to pay
  the receivers back yourself.
</Callout>
