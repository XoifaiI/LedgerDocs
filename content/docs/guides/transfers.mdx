...
<Callout type="warn">
  `Store:Erase(Key)` hands over any money the key was sending out. For 8 days after that it refuses
  anything sent to the key, so those senders get refunded rather than losing it. Sending to an erased
  key answers `Refused`. If the handover itself fails, it warns with how much was lost and you'll
  have to pay the receivers back yourself. See [Erase](/docs/guides/recovery#erase).
</Callout>
