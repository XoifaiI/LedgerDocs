...
```diff
+ Reserve costs no datastore read when the key had a hold in the last 10 minutes. The stock stays cached that long after the last hold runs out
+ Reap code tidied

! Apply answers Unresolved for every op while two or more transactions are parked on the key, even one your reducer refuses. Ask again once they settle
```
