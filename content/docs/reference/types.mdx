...
Your state with every field read only, at every depth. It's how the state you are handed is at
runtime. Annotate a reducer's state with it and a write into a nested table stops the build:
...
indexer read only yet. A write into one still throws while you play. In TypeScript `Ledger.Frozen`
...
type TypedConfig<D, O> = {
	read Name: string,
	read Reducer: (State: D, Op: Op<O>) -> (D | Frozen<D>)?,
	read Default: D,
	read Balance: string?,
	read Migrations: { Migration }?,
	read Keys: KeysMode?,
...
give back your state or `nil`. That state can be typed [`Frozen`](#frozen). See
