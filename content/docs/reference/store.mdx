...
Deletes the key. Hands over in flight transfer money first. Warns if the player is still in the
server, because their session will write part of it back.

`false` means the key is still there. Check it before you tell anyone their data is gone.
