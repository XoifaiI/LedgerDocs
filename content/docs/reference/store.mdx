...
you will `Confirm` on the same key. With a `To`, `Hold` has to be just under 30 days or less, because a
reservation held past the window Ledger keeps a delivery for is written off rather than given back.
