...
Throws the record away and leaves a tombstone, which turns away anything sent to that key for the
next 8 days so in flight money goes back to whoever sent it. Hands over money this key was sending
out first. Warns if the player is still in the server, because their session will write part of it
back. See [Erase](/docs/guides/recovery#erase).
