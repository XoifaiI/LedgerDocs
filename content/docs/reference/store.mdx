...
field of your state stops the build rather than throwing at the call. `Bump` and `Total` take the
same check.

Asking again under a name already set aside answers `true` and sets nothing aside a second time. Once
those units have gone, confirmed, released or given back, the same name answers
[`Spent`](/docs/concepts/reasons) and sets nothing aside.

Asking for more than the field holds answers `Refused`, and so does asking while a key already holds
256 reservations at once. That is far more than a key is meant to carry, so reaching it means they
are being made faster than they are being confirmed or released.
...
you will `Confirm` on the same key. With a `To`, `Hold` has to be just under 30 days or less, so that
a reservation still runs out inside the window Ledger can settle it in.

A reservation only keeps its units past 30 days if a `Grant` started handing it over and never
answered. Ledger cannot tell whether those units arrived, so it says so and leaves them for you to
sort out. One that was never handed over gets its units back however long it has been sitting.

A reservation nobody takes is given back 10 minutes after its hold runs out, either by the next
`Reserve` on that key or by the recovery sweep.
...
Answers [`Held`](/docs/concepts/reasons) while a `Grant` is handing those units over. They are not on
this key to spend, and only recovery can say where they ended up.

Once the op that confirmed it has been folded into the snapshot, asking again answers
...
Answers [`Held`](/docs/concepts/reasons) while a `Grant` is handing those units to another key. They
...
it gives anything back, so a reservation already handed over is settled rather than returned.
...
Both this and `RecoverTransfers` are about money moving between keys, so the store has to name a
`Balance` field. Calling either on a store without one throws where you called it.
