...
stops reading the top shards, and their bumps leave every total. During the deploy that raises it,
an old server sums only the shards it knows. Its totals are short until that server stops. A new
server keeps its own sum and reads every shard.
...
Throws the record away and leaves a tombstone. Money that the key was part way through sending goes
first. If Ledger cannot pass it on, `Erase` answers [`Busy`](/docs/concepts/reasons) and changes
nothing. Call it again later. A transfer that cannot be delivered is given back once it is 8 days
old, so an `Erase` after that goes through. For 8 days the tombstone refuses anything sent to the
key, so money still being sent returns to whoever sent it.
